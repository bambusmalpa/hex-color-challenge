...
const DificultyBar = (props) => {
  ...
  const levels=difLevels.map(el=>{
    return <li className={props.dificulty===el.dif?"dificultyBar__element dificultyBar__element--active":"dificultyBar__element"} key={el.dif} onClick={()=>props.setDificulty(el.dif)}>{el.name}</li>
  })
